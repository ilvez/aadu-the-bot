...
class SimpleClient < MatrixSdk::Client
  ...
  def handle_message(event)
    ...
    if %w[m.text m.notice].include? event.content[:msgtype]
      ...
      puts "[#{Time.now.strftime '%H:%M'}] <#{user.display_name}> #{"\033[1;30m" if notice}#{event.content[:body]}#{"\033[0m" if notice}"
    elsif event[:content][:msgtype] == 'm.emote'
      puts "[#{Time.now.strftime '%H:%M'}] *#{user.display_name} #{event.content[:body]}"
    else
      puts "[#{Time.now.strftime '%H:%M'}] <#{user.display_name}> (#{event.content[:msgtype]}) #{event.content[:body]} - #{api.get_download_url event.content[:url]}"
    end
  end
  ...
  begin
    ...
    loop do
      ...
      if msg.start_with? '/me'
        room.send_emote msg.gsub(%r{/\/me\s*/}, '')
      else
        room.send_text msg
      end
    end
